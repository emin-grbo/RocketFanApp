...
if let lastRunFile = try? scriptsFolder.file(named: ".payloadLastRun") {
...
//Download time!
let url = URL(string: "https://api.spacexdata.com/v3/payloads")!
let json = try Data(contentsOf: url)

//Writing file...
let resourceFolder = try Folder.current.subfolder(atPath: "RocketFan/Resources/Data")
try resourceFolder.createFile(named: "payload.json", contents: json)

//Save lastRun
let lastRunFile = try scriptsFolder.createFile(named: ".payloadLastRun")

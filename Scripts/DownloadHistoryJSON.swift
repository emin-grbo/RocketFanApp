...
if let lastRunFile = try? scriptsFolder.file(named: ".historyLastRun") {
...
//Download time!
let url = URL(string: "https://api.spacexdata.com/v3/history")!
let json = try Data(contentsOf: url)

//Writing file...
let resourceFolder = try Folder.current.subfolder(atPath: "RocketFan/Resources/Data")
try resourceFolder.createFile(named: "history.json", contents: json)

//Save lastRun
let lastRunFile = try scriptsFolder.createFile(named: ".historyLastRun")
